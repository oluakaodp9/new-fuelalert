...
$filling_stationCheck = $filling_station->get_filling_station();

$filling_station_to_delete = $filling_stationCheck->fetch(PDO::FETCH_ASSOC);
// var_dump($filling_station_to_delete);
// return;

if(!$filling_station_to_delete){
    // No valid filling_station id provided
    ...
    // tell the filling_station no products found
    echo json_encode(
        array("message" => "filling_station with ID:$filling_station->id does not exist")
    );

    return;
}
  
// delete the filling_station
if($filling_station->delete_filling_station()){
  
    // set response code - 200 ok
    http_response_code(200);
  
    // tell the filling_station
    echo json_encode(array("message" => "filling_station was deleted successfully."));
}
  
// if unable to delete the filling_station
else{
    ...
    // tell the filling_station
    echo json_encode(array("message" => "Unable to delete filling_station."));
}
?>